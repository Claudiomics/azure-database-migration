...
The following steps outline the creation of a production environment to restore the Microsoft 'AdventureWorks' production database, which is a sample database provided by Microsoft emulating a fictional manufacturing company's operations, effecitvely replicating a real-world production database scenario. I chose the Windows 11 Pro operating system as my Virtual Machine (VM) as this setup simulates a secure and dedicated data storage solution that is likely what an on-premise system within a company would use for this type of database. Running a Windows 11 VM on my local machine offers several advantages such as being able to access exclusive windows applications and software, and being able to run this software without the need for a seperate physical windows machine. To enable incoming network traffic from my local port specifically when creating my virtual machine, I needed to create an inport bound rule which would allow me to connect through the Remote Desktop Protocol (RDP), as, by default, all inbound traffic is blocked for security purposes.

I hosted my database on Azure SQL as it is a cloud-based relational database service providing a managed and scalable platform for running SQL Server databases on the cloud. SQL Server is a relational database mamagement system (RDBMS) developed by Microsoft to store and manage vast quantities of data and supports structured query language (SQL). SQL Server Management Studio (SSMS) is a powerful graphical user interface (GUI) tool for managing SQL Server instances and databases. Azure SQL offers many of the same features and functionality as SQL Server but with the added benefits of cloud computing, as well as enhanced performance, scalability and security. I needed to install SQL Server and SSMS onto my production VM to manage the AdventureWorks database. SSMS is then the central hub for database administrators and developers to perform various tasks relating to SQL Server. I downloaded the database onto SQL Server using SSMS's backup option after downloading the .bak file.
...
1.	Firstly, I downloaded the Production Database AdventureWorks from the [file](https://aicore-portal-public-prod-307050600709.s3.eu-west-1.amazonaws.com/project-files/93dd5a0c-212d-48eb-ad51-df521a9b4e9c/AdventureWorks2022.bak) provided by Mcirosoft onto my VM.
